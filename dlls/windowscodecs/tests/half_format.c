/* Half-float pixel format registration and bitmap storage regressions.
...
static void test_half_formats(void)
{
    static const struct {const GUID *format;UINT bits,channels;BOOL alpha;} formats[]={
        {&GUID_WICPixelFormat64bppRGBAHalf,64,4,TRUE},
        {&GUID_WICPixelFormat64bppPRGBAHalf,64,4,TRUE},
        {&GUID_WICPixelFormat48bppRGBHalf,48,3,FALSE},
        {&GUID_WICPixelFormat16bppGrayHalf,16,1,FALSE},
        {&GUID_WICPixelFormat128bppPRGBAFloat,128,4,TRUE},
    };
    ...
    for(f=0;f<ARRAY_SIZE(formats);++f)
    {
        ...
        ok(numeric==WICPixelFormatNumericRepresentationFloat,"Got representation %u.\n",numeric);
        ...
        for(c=0;c<channels;++c)
        {
            ...
            hr=IWICPixelFormatInfo2_GetChannelMask(info,c,1,mask,&actual);
            ok(hr==E_INVALIDARG && actual==bytes,"Short mask: %#lx, %u.\n",hr,actual);
            ok(mask[0]==0xcc,"Modified short mask.\n");
            memset(expected,0xcc,sizeof(expected));memset(expected,0,bytes);
            memset(expected+c*(f==4?4:2),0xff,f==4?4:2);
