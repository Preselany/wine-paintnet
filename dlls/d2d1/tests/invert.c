/* Tests for Direct2D color inversion, including feature level 10.
...
static void test_invert(BOOL fl10)
{
    ID3D11Device *d3d;
    IDXGIDevice *dxgi;
    ...
    ID2D1Bitmap1 *source, *target, *readback, *source_readback;
    ...
    const GUID *ids[] = {&CLSID_D2D1Invert,&CLSID_D2D1Invert};
    ...
    for (mode=1;mode<=3;++mode)
    {
                ...
                for(i=0;i<64;++i)
                {
                    ...
                    if(i%4 != 3)
                    {
                        ...
                        else
                        {
                            ...
                            if(chain)expected=alpha-expected;
                        }
                    }
                    ok(isfinite(value) && fabsf(value-expected) <= max(5e-8f,fabsf(expected)*1e-6f),
        ...
        desc.bitmapOptions=D2D1_BITMAP_OPTIONS_CPU_READ | D2D1_BITMAP_OPTIONS_CANNOT_DRAW;
        REQUIRE(ID2D1DeviceContext_CreateBitmap(dc,size,NULL,0,&desc,&source_readback));
        REQUIRE(ID2D1Bitmap1_CopyFromBitmap(source_readback,NULL,(ID2D1Bitmap *)source,NULL));
        REQUIRE(ID2D1Bitmap1_Map(source_readback,D2D1_MAP_OPTIONS_READ,&mapped));
        ok(!memcmp(mapped.bits,pixels,sizeof(pixels)),"Invert changed source pixels.\n");
        REQUIRE(ID2D1Bitmap1_Unmap(source_readback));
        ID2D1Bitmap1_Release(source_readback);
