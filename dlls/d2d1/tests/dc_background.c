...
START_TEST(dc_background)
{
    ...
    for(alpha=0;alpha<2;++alpha)for(path=0;path<2;++path)
    {
        ...
        for(stage=0;stage<3;++stage)
        {
            ...
            GdiFlush();
            winetest_push_context("alpha %u, interface %u, draw %u", alpha, path, stage);
            ok(hr == S_OK, "EndDraw returned %#lx.\n", hr);
            for (i = 0; i < 16 * 12; ++i)
            {
                x = i % 16; y = i / 16;
                expected = stage && x >= 3 && x < 5 && y >= 4 && y < 6
                        ? 0xffff0000 : 0xff204060 + stage * 0x102030 + i;
                ok(pixels[i] == expected, "Pixel %u,%u: %#lx, expected %#lx.\n", x, y, pixels[i], expected);
            }
