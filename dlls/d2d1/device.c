...
static const char shape_ps_code[] =
    ...
    "};\n"
    "\n"
    "float4 sample_gradient(Buffer<float4> gradient, uint extend_mode, float position)\n"
    "{\n"
    "    float footprint = length(float2(ddx(position), ddy(position)));\n"
    "    uint size = (uint)clamp(exp2(ceil(log2(1.0 / max(footprint, 1.0e-20) + (extend_mode == 0 ? 2.0 : 0.0)))), 4.0, 1024.0);\n"
    "    float x, fraction;\n"
    "    uint index, next, offset = size - 4;\n"
    "\n"
    "    if (extend_mode == 1)\n"
    "        position = frac(position);\n"
    "    else if (extend_mode == 2)\n"
    "        position = 1.0 - abs(2.0 * frac(position * 0.5) - 1.0);\n"
    "    x = extend_mode == 0 ? position * (size - 2) + 0.5 : position * size - 0.5;\n"
    "    if (extend_mode == 1)\n"
    "        x = x < 0.0 ? x + size : x;\n"
    "    else\n"
    "        x = clamp(x, 0.0, (float)(size - 1));\n"
    "    index = (uint)floor(x);\n"
    "    next = index + 1;\n"
    "    if (extend_mode == 1)\n"
    "    {\n"
    ...
    "    else\n"
    "    {\n"
    "        next = min(next, size - 1);\n"
    "    }\n"
    "    fraction = floor(frac(x) * 128.0) / 128.0;\n"
    "    return lerp(gradient.Load(offset + index), gradient.Load(offset + next), fraction);\n"
    "}\n"
    "\n"
    ...
    "    end = brush.data[0].zw;\n"
    "    extend_mode = asuint(brush.data[1].x);\n"
    "\n"
    "    v_p = position - start;\n"
    "    v_q = end - start;\n"
    "    p = dot(v_q, v_p) / dot(v_q, v_q);\n"
    "\n"
    "    return sample_gradient(gradient, extend_mode, p);\n"
    "}\n"
    "\n"
    ...
    "    ra = brush.data[1].xy;\n"
    "    rb = brush.data[1].zw;\n"
    "    extend_mode = asuint(brush.data[2].x);\n"
    "\n"
    "    /* Project onto ra, rb. */\n"
    ...
    "    t = -b + sqrt(b * b - c);\n"
    "\n"
    "    return sample_gradient(gradient, extend_mode, l / t);\n"
    "}\n"
    "\n"
