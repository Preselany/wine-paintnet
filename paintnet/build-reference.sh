...
for probe in emboss-reference emboss-grid8-reference effect-properties-reference animation-reference animation-keyframe-reference shader-reflection-reference draw-transform-reference command-list-reference command-context-reference color-context-reference color-context-icc-reference effect-node-reference alpha-conversion-reference dispatcher-queue-reference; do
    probe_source="$probe"
    probe_flags=()
    probe_libs=()
    if [[ "$probe" == dispatcher-queue-reference ]]; then
        probe_flags=(-Wno-unused-parameter)
        probe_libs=(-Ldlls/coremessaging/x86_64-windows -Ldlls/combase/x86_64-windows -lcoremessaging -lcombase -luser32)
    fi
    ...
    docker run --rm --user "$(id -u):$(id -g)" \
        ...
        "${probe_libs[@]}" -ld2d1 -ld3d11 -ld3dcompiler -ldxguid -luuid -lole32
