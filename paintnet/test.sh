...
if [[ $# == 0 ]]; then set -- effect_identity effect_context effect_node alpha_conversion histogram opacity_metadata alpha_mask convolve_matrix contrast bitmap_source emboss opacity draw_transform command_list_state color_context timeline keyframes uianimation requirements coremessaging; fi
