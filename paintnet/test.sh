...
for test_name in "$@"; do
    module=d2d1
    if [[ "$test_name" == pixel_color_context || "$test_name" == half_format || "$test_name" == alpha_format || "$test_name" == float_conversion ]]; then module=windowscodecs; fi
    if [[ "$test_name" == font_cache ]]; then module=dwrite; fi
