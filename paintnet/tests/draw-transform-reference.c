/* Public-API measurements for custom Direct2D pixel shader transforms.
...
#define REQUIRE(call) do { hr = (call); if (FAILED(hr)) { printf("FAILED %s %08lx\n",#call,hr); goto done; } } while (0)
    const char *selected_shader = getenv("D2D1_PROBE_SCENE")
            ? "float4 main(float4 pos:SV_POSITION,float4 scene:SCENE_POSITION):SV_TARGET {return scene;}" : shader;
