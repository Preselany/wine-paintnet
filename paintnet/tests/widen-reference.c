/* Native bounds, area, and pixels for contained geometry combinations.
...
int main(void)
{
    ...
    const BOOL app_shape = !!getenv("WIDEN_PAINTNET");
    ...
    ID2D1GeometrySink_EndFigure(sink,getenv("WIDEN_OPEN") ? D2D1_FIGURE_END_OPEN : D2D1_FIGURE_END_CLOSED);
    ...
    for(shape=app_shape ? 1 : 0;shape<(app_shape ? 2 : 3);++shape)for(transformed=0;transformed<2;++transformed)for(mode=0;mode<4;++mode)
    {
