...
int main(void)
{
    ...
    GUID format;DWORD crc;UINT count;unsigned int when=1, variant;const unsigned int sizes[]={0,1,4,7,8,9,22};HRESULT hr;ULARGE_INTEGER size;
    ...
    for(variant=0;variant<ARRAY_SIZE(sizes);++variant)
    {
        printf("case %u\n",sizes[variant]);
        ...
        value.vt=VT_BLOB;value.blob.cbSize=sizes[variant];value.blob.pBlobData=chunk;
        ...
        hr=IWICBitmapFrameEncode_WritePixels(frame,1,8,sizeof(pixels),pixels);printf("write pixels %08lx\n",hr);
