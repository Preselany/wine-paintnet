...
static struct draw_effect *source_latest;
static ID3DBlob *code;
static D2D1_RECT_L source_bounds = {-2,-1,6,5};
        ...
        D2D1_RECT_L *inputs, UINT32 count)
{
    ...
    ++mapped_outputs;
    printf("output %ld %ld %ld %ld %u\n",output->left,output->top,output->right,output->bottom,count);
...
static HRESULT CALLBACK source_factory(IUnknown **out)
{
...
static const WCHAR source_xml[] = L"<?xml version='1.0'?><Effect>"
    L"<Property name='DisplayName' type='string' value='Offset source'/>"
    L"<Property name='Author' type='string' value='Wine'/>"
    L"<Property name='Category' type='string' value='Test'/>"
    L"<Property name='Description' type='string' value='Shifted custom source'/><Inputs/></Effect>";
int main(void)
{
    static const char shader[]="float4 main(float4 p:SV_POSITION,float4 s:SCENE_POSITION):SV_TARGET"
            "{return float4(s.x*.05,s.y*.1,.25,1);}";
    ...
    ID2D1Bitmap1 *target,*readback;ID2D1Effect *source,*outer,*opacity;ID2D1Image *source_image;ID3DBlob *errors=NULL;
    ...
    struct wrapper *w;HRESULT hr;unsigned int extent,dpi,units,path,i;float alpha=.5;
    ...
    REQUIRE(D3DCompile(shader,sizeof(shader)-1,NULL,NULL,NULL,"main","ps_4_0",0,0,&code,&errors));
    if(errors)ID3D10Blob_Release(errors);
    REQUIRE(ID2D1Factory1_RegisterEffectFromString(factory,&CLSID_ProbeDraw,source_xml,NULL,0,source_factory));
    ...
    for(extent=0;extent<2;++extent)
    {
    source_bounds=extent ? (D2D1_RECT_L){(-2147483647-1),(-2147483647-1),2147483647,2147483647}
            : (D2D1_RECT_L){-2,-1,6,5};
    REQUIRE(ID2D1DeviceContext_CreateEffect(dc,&CLSID_ProbeDraw,&source));
    ID2D1Effect_GetOutput(source,&source_image);
    ID2D1Effect_SetInput(outer,0,source_image,TRUE);
    ID2D1Image_Release(source_image);
    for(dpi=0;dpi<ARRAY_SIZE(dpis);++dpi)for(units=0;units<2;++units)for(path=0;path<3;++path)
    {
        ...
        for(i=0;i<ARRAY_SIZE(offsets);++i)
        {
            ...
            printf("case %u %u %u %u %u\noffset %ld %ld\n",extent,dpi,units,path,i,actual.x,actual.y);
            if (render(dc,outer,target,readback)) return 1;
            fflush(stdout);
        }
    }
    ID2D1Effect_Release(source);
    }
    ...
    ID2D1DeviceContext_SetTarget(dc,NULL);ID2D1Bitmap1_Release(readback);ID2D1Bitmap1_Release(target);
    ID3D10Blob_Release(code);
    ID2D1Factory1_UnregisterEffect(factory,&CLSID_ProbeDraw);
