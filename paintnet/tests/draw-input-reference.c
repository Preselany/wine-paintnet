...
static void validate(ID2D1DrawInfo *info)
{
    ...
    for (p = 0; p <= 6; ++p) for (d = 0; d < sizeof(depths)/sizeof(*depths); ++d)
        printf("buffer %u %u %08lx\n",p,depths[d],ID2D1DrawInfo_SetOutputBuffer(info,p,depths[d]));
    for (i = 0; i < sizeof(sizes)/sizeof(*sizes); ++i)
        printf("constant %u %08lx\n",sizes[i],ID2D1DrawInfo_SetPixelShaderConstantBuffer(info,data,sizes[i]));
    printf("constant_null_zero %08lx\n",ID2D1DrawInfo_SetPixelShaderConstantBuffer(info,NULL,0));
    printf("missing_shader %08lx\n",ID2D1DrawInfo_SetPixelShader(info,&CLSID_ProbeDraw,0));
    for (i = 0; i < 5; ++i)
        printf("shader_options %u %08lx\n",i,ID2D1DrawInfo_SetPixelShader(info,&GUID_ProbeShader,i));
    printf("input_zero %08lx\n",ID2D1DrawInfo_SetInputDescription(info,0,input));
    for (i = 0; i < 24; ++i)
    {
        input.filter = i;
        printf("filter %u %08lx\n", i, ID2D1DrawInfo_SetInputDescription(info,0,input));
    }
...
int main(void)
{
    ...
    char warp[2];
    D3D_DRIVER_TYPE driver = GetEnvironmentVariableA("D2D1_TEST_WARP",warp,sizeof(warp)) ? D3D_DRIVER_TYPE_WARP : D3D_DRIVER_TYPE_HARDWARE;
    setvbuf(stdout,NULL,_IOFBF,65536);
    CoInitializeEx(NULL,COINIT_MULTITHREADED);
#define REQUIRE(call) do { hr = (call); if (FAILED(hr)) { printf("FAILED %s %08lx\n",#call,hr); goto done; } } while (0)
    const char *selected_shader = getenv("D2D1_PROBE_COORDINATES")
            ? "float4 main(float4 pos:SV_POSITION,float4 scene:SCENE_POSITION,float4 tc:TEXCOORD0):SV_TARGET {return tc;}"
            : shader;
    REQUIRE(D3DCompile(selected_shader,strlen(selected_shader),NULL,NULL,NULL,"main","ps_4_0",0,0,&code,&errors));
    ...
    if (getenv("D2D1_PROBE_CACHED")) ID2D1DrawInfo_SetCached(latest->info,TRUE);
    ...
    for (p = 0; p <= 5; ++p) for (d = 0; d <= 4; d += d ? 3 : 1) for (variant = 0; variant < 4; ++variant)
    {
        ...
        printf("bounds %u %u %u %08lx %.9g %.9g %.9g %.9g\n",p,d,variant,hr,rect.left,rect.top,rect.right,rect.bottom);
        ...
        printf("draw %u %u %u %08lx %u %u %u\n",p,d,variant,hr,prepared,mapped_inputs,mapped_outputs);
        ...
        printf("pixels %u %u %u",p,d,variant);
